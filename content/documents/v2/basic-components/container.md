...
Swoft 基于 `PSR-11` 规范设计了自己容器，并基于 **注解** 增强了它的功能。容器是 Swoft 最重要的设计，称得上是 Swoft 的核心精髓，也是 Swoft 各种模块的实现基础。本章节将会对容器的相关基础知识做一个介绍，以便朋友们更好的理解容器。

## 前置知识 - IoC

IoC 即控制反转（Inversion of Control），它不是一门技术而是一种设计思想。利用 IoC 将你设计好的对象交给容器控制，而非传统地在你的对象内直接控制、处理。简单来说就是如此，但不便于理解，我们继续下文。

## 前置知识 - DI

DI 为依赖注入（Dependency Injection）的缩写，DI 其实也不是一门技术，它是一种实现 IoC 的方式。两个名词先交代在这里，我们继续阅读下方图文说明。

## 概念知识

我们知道一台标准的电脑拥有 USB 接口，通过 USB 接口我们可以扩展额外的能力，如：键盘、鼠标、U 盘、蓝牙、无线网卡等等，只要是 USB 接口的设备，连接到电脑上就能正常工作并实现其输入输出功能。现在我们有一台电脑，可惜并没有 USB 接口，但我希望能够使用鼠标、键盘和 U 盘，所以我必须 **依赖** USB 接口来实现我们的“愿望”。
...
我们通过一些伪代码来理解上述的 IoC 及 DI，首先是我们的电脑：
